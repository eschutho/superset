    ...
    def slack_channels(self, **kwargs: Any) -> Response:
        ...
        try:
            ...
            if force:
                ...
                # Trigger async cache warmup if caching is enabled. This is a
                # best-effort side optimization, not the purpose of this
                # request, so a broker that's unreachable/unconfigured must
                # not turn an otherwise-successful synchronous fetch into a
                # 500 - log and continue either way.
                if current_app.config.get("SLACK_ENABLE_CACHING", True):
                    try:
                        ...
                        logger.warning(
                            "Could not trigger async Slack channels cache "
                            "warmup (Celery broker unreachable): %s. "
                            "Continuing with a synchronous fetch for this "
                            "request.",
                            str(ex),
                        )
                    except Exception as ex:  # noqa: BLE001
                        logger.warning(
                            "Failed to trigger async Slack channels cache "
                            "warmup: %s",
                            str(ex),
                        )
