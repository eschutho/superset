...
|------------|-----------|-------------|
| `can_export_data` | Superset | Controls data exports (CSV, Excel, JSON). Users without this permission cannot download data from charts, dashboards, or SQL Lab. |
| `can_export_image` | Superset | Controls image and PDF exports. Users without this permission cannot download charts or dashboards as images or PDFs. |
| `can_copy_data` | Superset | Controls clipboard functionality. Users without this permission cannot copy data to clipboard from SQL Lab or Explore. |

**Note**: The legacy `can_csv` permission is still supported for backward compatibility and works the same as `can_export_data`.

To restrict export capabilities for a role:

1. Navigate to **Security** > **List Roles**
2. Select the role you want to modify
3. In the Permissions dropdown, remove the relevant export permission (`can_export_data on Superset`, `can_export_image on Superset`, or `can_copy_data on Superset`)
4. Save the role
